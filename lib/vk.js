...
module.exports = class VK{
    constructor(params = {}){
        ...
        this.getMessageAttachments = this.getMessageAttachments.bind(this);
    ...
    async init(){
        ...
        if(this.account_type === 'community'){
            this.ccallback_api = new ccallback_api(params);
    ...
    async getMessageAttachments(id){
        debug('getMessageAttachments', id);
        const data = await this.call("messages.getById", {message_ids: id});
        if(!data[1])
            return [];
        return data[1].attachments;
    }
