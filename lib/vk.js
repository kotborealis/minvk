...
module.exports = function(init){
    ...
    this.call = (name,param,callback)=>{
        ...
        request.post(`https://api.vk.com/method/${name}`,{form:param},(err,res)=>{
            if(err) {
                console.log("call network error");
                callback(err, null);
            }
            else{
                const data = JSON.parse(res.body);
                if(data.error) {
                    console.log("call api error",data.error,data,name,param);
    ...
    this.longPoll = (callback)=>{
        console.log("called longPoll");
        if(!callback) {
            console.log("called longPoll w/o callback");
            return;
        }
        this.call("messages.getLongPollServer",{},(err,res)=>{
            if(err) {
                console.log("longpoll getLongPollServer error", err, res);
                callback(res, null);
            }
            else
    ...
    const longPollHelper = (server,key,ts,callback)=>{
        console.log("called longPollHelper");
        request(`https://${server}?act=a_check&key=${key}&ts=${ts}&wait=25&mode=2`,{timeout:25000},(err,res)=>{
            if(err) {
                console.log("longPollHelper network error");
                this.longPoll(callback);
            }
            else{
                const data = JSON.parse(res.body);
                console.log("longPollHelper data",data);
                if(data.failed) {
                    console.log("longPollHelper failed",data.failed);
                    this.longPoll(callback);
                }
                else {
                    ts = data.ts;
