        ...
        request(`https://${server}?act=a_check&key=${key}&ts=${ts}&wait=25&mode=2`,{timeout:25000},(err,res)=>{
            if(err) {
                ...
                try {
                    const data = JSON.parse(res.body);
                    if (data.failed) {
                        console.log("longPollHelper failed", data.failed, data);
                        this.longPoll(callback);
                    }
                    else {
                        ts = data.ts;
                    ...
                    console.log("longPollHelper error, can not parse res.body as JSON", res.body);
                    console.log("Trying to get new longPoll server");
                    this.longPoll(callback);
                }
            }
